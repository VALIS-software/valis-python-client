...
class QueryBuilder:
  ...
  def filterID(self, doc_id):
    copy = self.duplicate()
    copy.query['filters']['_id'] = doc_id
    return copy
  ...
  def fetch(self, full=False, startIdx=None, endIdx=None, limit=None):
    query = self.setLimit(limit) if limit is not None else self
    result, _ = self.api.getQueryResults(query, full, startIdx, endIdx)
    return result
