from valis import ValisAPI, Dataset
...
for pathway in pathways[:3]:
	...
	genes_in_pathway = client.genomeQuery().filterType('gene').filterPathway(pathway)
	...
	variants = client.genomeQuery().filterType('SNP').filterSource(Dataset.DBSNP)
	eQTLs_near_pathway = variants.addToEdge(eQTLs.toNode(genes_in_pathway))
	
	# find which of these are within 100bp of roadmap epigenomics annotations: 
